...
def cargar_datos_dashboard():
    df = pd.read_csv("/mnt/data/1.inf tecnicos PEI 2016-2017-2018-2019-2020-2021-2022...V4..xlsx - Dash_Data_UEs.csv")
    return df

# -------------------------
# GRAFICO PDC - BARRAS POR NIVEL
# -------------------------
def mostrar_detalle_pdc():
    df = cargar_datos_dashboard()
    df_pdc = df.iloc[0:2]  # Solo niveles para PDC

    df_pdc["Entidades Con PDC"] = pd.to_numeric(df_pdc["Entidades Con PDC"], errors='coerce')
    df_pdc["Entidades Sin PDC"] = pd.to_numeric(df_pdc["Entidades Sin PDC"], errors='coerce')

    fig = go.Figure()

    fig.add_trace(go.Bar(
        y=df_pdc["Nivel de Gobierno"],
        x=df_pdc["Entidades Con PDC"],
        name="Formulados",
        orientation='h',
        marker_color="#308446"
    ))
    ...
    fig.update_layout(
        barmode='stack',
        title="PDC por Nivel de Gobierno",
        xaxis_title="Número de Pliegos",
    ...
    <div style='text-align: center; margin-top: 20px;'>
        <button onclick="window.location.reload();" style="padding: 0.5em 1em; font-size: 16px; border-radius: 6px; border: none; background-color: #f0f0f0; cursor: pointer;">
...
st.markdown("""
### Selecciona plan para mapa:
""")

plan_sel = st.radio("", ["PEI", "POI", "PDC"], horizontal=True)

# -------------------------
# MOSTRAR GRÁFICO POR NIVEL SI ES PDC
# -------------------------
if plan_sel == "PDC":
    mostrar_detalle_pdc()

# (aqui iría el resto de tu código, como KPI, mapa, etc)
# Por ejemplo:
# render_map(plan_sel)
# kpi_card(...)
# resumen_grafico(...)
